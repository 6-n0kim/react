  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-neutral-700/80 backdrop-blur-lg">
      <div className="container relative">
          ...
          <div className="info-wrapper items-center gap-2 hidden lg:flex">
            {authLink.map((item, idx) => (
              <Link
                key={idx}
                to={item.to}
                className={`py-2 px-3 rounded-md${idx === 1 ? "bg-gradient-to-r from-indigo-400 to-indigo-800 text-neutral-200" : "border border-neutral-800/80"}`}
              >
                {item.label}
              </Link>
            ))}
        ...
        {mobileOpen && (
          <div className="absolute right-0 z-50 bg-white w-full pb-0 flex top-full flex-col justify-center items-center dark:bg-neutral-900 border-neutral-700 mt-1">
             <ul className="w-full">
              {navItems.map((item, idx) => (
                <li key={idx} className="py-4 border-b border-neutral-700 w-full text-center lg:hidden">
